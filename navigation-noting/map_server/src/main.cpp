...
class MapServer
{
  public:
    ...
    MapServer(const std::string& fname, double res)
    {
      ...
      deprecated_ = (res != 0);
      if (!deprecated_) {
        if (!loadMapFromYaml(fname))//% 从yaml文件加载地图
        {
          exit(-1);
        }
      } else {
        if (!loadMapFromParams(fname, res))//% 从参数加载地图
        {
          exit(-1);
        }
      }
  ...
  private:
    ...
    //￥ 无论是loadMapFromYaml，loadMapFromParams 最终都是调用此函数加载地图。
                           ...
                           double origin[3], MapMode mode)
    {
      ...
      try {
        //￥ 通过map_server类中的loadMapFromFile函数加载地图
      ...
      metadata_pub_.publish( meta_data_message_ );//￥ 发布地图元数据
      map_pub_.publish( map_resp_.map );//￥ 发布地图数据
      return true;
    }
