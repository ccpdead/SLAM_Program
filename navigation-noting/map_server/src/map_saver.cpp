    ...
    void mapCallback(const nav_msgs::OccupancyGridConstPtr& map)
    {
      ...
      for(unsigned int y = 0; y < map->info.height; y++) {
        for(unsigned int x = 0; x < map->info.width; x++) {
          ...
          if (map->data[i] >= 0 && map->data[i] <= threshold_free_) { //% [0,free) 空白区域
            fputc(254, out);
          } else if (map->data[i] >= threshold_occupied_) { //% (occ,255] 障碍物区域
            fputc(000, out);
          } else { //%occ [0.25,0.65] 未知区域
            fputc(205, out);
          }
        }
      }
      ...
      mat.getEulerYPR(yaw, pitch, roll);//￥ 将四元数转化为欧拉角
