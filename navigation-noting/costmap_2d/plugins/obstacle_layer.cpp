...
void ObstacleLayer::onInitialize()
{
  ...
  //$ 跟踪未知空间，注意，若不设定此处，则 global_planner中的allow_Known参数无效
  nh.param("track_unknown_space", track_unknown_space, layered_costmap_->isTrackingUnknown());
  
  //$ 若设置了track_unknown_space则将未探测区域设置为不可见，否则设置为可用空间
  ...
  //% observation source设置了movebase参考的传感器
  ...
  while (ss >> source)
  {
    ...
    //￥ movebase只能接收pointcloud2,pointcloud,laserscan这三种数据
    ...
    if (sensor_frame != "")
    {
      ...
      target_frames.push_back(global_frame_);//全局参考系
      target_frames.push_back(sensor_frame);//传感器参考系
...
/**
 * @brief 对不同的sensor类型注册不同的回调函数，LaserScan,PointCloud2,PointCloud
 * 
 * @param message 
 * @param buffer 
 */
                                      ...
                                      const boost::shared_ptr<ObservationBuffer>& buffer)
{
  ...
  buffer->lock();
  //$ 此函数位于observation_buffers.cpp中 主要通过pcl对雷达点数据进行一系列的操作。
                                          ...
                                          double* min_y, double* max_x, double* max_y)
{
  ...
  {
    //$ 清除传感器到障碍物之间的区域，并将此区域标记为free space
